...
Constraint.prototype.enforce = function() {
  ...
  let v12 = new THREE.Vector3(0.0, 0.0, 0.0);
  v12 = v12.subVectors(this.p2.position, this.p1.position);
  let mag_v12 = v12.length();
  let correction = v12.multiplyScalar((mag_v12 - this.distance) / mag_v12);
...
function Cloth(w, h, l) {
  ...
  if (structuralSprings) {
    ...
    for (i = 0; i <= h; i++) {
      for (j = 0; j <= w; j++) {
        ...
        if (i < h) {
          constraints.push(
            new Constraint(particles[index(j, i)], particles[index(j, i + 1)], restDistance)
          );
        }

        if (j < w) {
          constraints.push(
            new Constraint(particles[index(j, i)], particles[index(j + 1, i)], restDistance)
          );
        }
  ...
  if (shearSprings) {
    ...
    let restDistanceShear = restDistance * restDistanceS;
    for (i = 0; i <= h - 1; i++) {
      for (j = 0; j <= w - 1; j++) {
        constraints.push(
          new Constraint(particles[index(j, i)], particles[index(j + 1, i + 1)], restDistanceShear)
        );
      }
    }
    ...
    for (i = 0; i < h; i++) {
      for (j = 0; j < w; j++) {
        constraints.push(
          new Constraint(particles[index(j + 1, i)], particles[index(j, i + 1)], restDistanceShear)
        );
      }
    }
  ...
  if (bendingSprings) {
    ...
    let restDistanceBending = restDistance * restDistanceB;

    for (i = 0; i <= h; i++) {
      for (j = 0; j <= w - 2; j++) {
        constraints.push(
          new Constraint(particles[index(j, i)], particles[index(j + 2, i)], restDistanceBending)
        );
      }
    }
    ...
    for (i = 0; i <= h - 2; i++) {
      for (j = 0; j <= w; j++) {
        constraints.push(
          new Constraint(particles[index(j, i)], particles[index(j, i + 2)], restDistanceBending)
        );
      }
    }
    // ----------- STUDENT CODE END ------------
...
Cloth.prototype.applyGravity = function() {
  ...
  for (var i = 0; i < particles.length; i++) {
    let grav = new THREE.Vector3(0.0, -1.0 * GRAVITY, 0.0);
    let force = grav.multiplyScalar(particles[i].mass);
...
Cloth.prototype.applyWind = function() {
  ...
  windStrength = Math.sin(time / 500);
  windStrength++;
  if (windStrength > 0.5) {
    windForce.setX(20 * windStrength);
  }
  windForce.setY(30 * windStrength);
